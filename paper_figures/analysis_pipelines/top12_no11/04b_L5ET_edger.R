setwd("~/cdkwan/alex_kwan/paper_figures/figures/top12_no11/figure_3/pseudobulk_clusters_counts/")
...
for (this.time in c('1h', '2h','4h','24h','72h')) {
  for (this.drug in c('Ket','Psilo')) {
    ...
    write.csv(final.table, file=paste0('DEGs_', this.time, '_', this.drug, '.csv'))

    kwanlibr::make_volcano(
      lrt,
      figure_title = paste0(this.time, ' ', this.drug, ' L5-PT cell DEGs'),
